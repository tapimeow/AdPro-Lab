...
public class GameCharacter extends Pane {
    ...
    public GameCharacter (String spritePath, int count, int columns, int rows, int offsetX, int offsetY, int width, int height,
                          int x, int y, int xMaxVelocity, int yMaxVelocity, KeyCode leftKey, KeyCode rightKey, KeyCode upKey) {
        this.x = x;
        this.y = y;
        ...
        this.setTranslateX(x);
        this.setTranslateY(y);
        this.gameCharacterImg = new Image(Launcher.class.getResourceAsStream(spritePath));
    ...
    public void checkReachGameWall() {
        if (x <= 0) {
            x = 0;
            logger.debug("Character collided with left wall: x={}", x);
        } else if (x + getWidth() >= GameStage.WIDTH) {
            x = GameStage.WIDTH-(int)getWidth();
            logger.debug("Character collided with right wall: x={}", x);
        }
    }
    ...
    public void checkReachFloor() {
        int currentHeight = (imageView != null) ? (int) imageView.getFitHeight() : CHARACTER_HEIGHT;
        if (isFalling && y >= GameStage.GROUND - currentHeight) {
            isFalling = false;
            canJump = true;
            yVelocity = 0;
            logger.debug("Character collided with floor: y={}", y);
        }
    }
